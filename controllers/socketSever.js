...
const handleSocketEvents = (socket) => {
  ...
  socket.on("sendMessage", async (data) => {
    ...
    //neu targett la  gr
    const group=await Group.findOne({ Groupname: data.usertarget }).populate('members');
    if(!recei){
  ...
  socket.on("getMessage", async (data) => {
    ...
    if(getavt){
      ...
      const messages = await MessageGroup.find({ receiver: group._id }).sort({ timestamp: 1 }).populate(['sender','receiver']);
  ...
  if(!targettarget){
    ...
    await MessageGroup.deleteMany({ sender: Login._id, receiver: target._id });
    const group=await Group.findOne({Groupname:targetUser})
    const messages = await MessageGroup.find({ receiver: group._id }).sort({ timestamp: 1 }).populate(['sender','receiver']);
    socket.emit("loadmessGroup", { messages, targetUser,avt:group.avatar}); 
  }
